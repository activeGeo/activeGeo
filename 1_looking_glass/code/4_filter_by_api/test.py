...
requests_get = partial(requests.get, timeout=6, verify=False)
requests_post = partial(requests.post, timeout=6, verify=False)
...
input_list = pickle.load(open(f'./result/list_bad_routers.bin', 'rb'))
...
pickle.dump(list_good_routers, open(f'./list_good_routers.bin', 'wb'))
pickle.dump(list_bad_routers, open(f'./list_bad_routers.bin', 'wb'))
